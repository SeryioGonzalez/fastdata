...
az group deployment create --resource-group $resGroupName  --name $hdiSparkDeploymentName --template-file $templateHDISpark --parameters \
	baseClusterName=$environmentName \
	clusterLoginPassword=$hdiClusterHTTPPassword \
	clusterLoginUserName=$hdiClusterHTTPUser \
	clusterWorkerNodeSize=$hdiSparkWorkerNodeSize \
